...
struct Args {
    ...
    #[arg(
        ...
        long,
        default_value = "001",
        help = "Number suffix for output files (e.g., 001, 002)"
    )]
...
fn reader_thread(r1_path: &Path, r2_path: &Path, batch_len: usize, tx: BatchSender) -> Result<()> {
    // 构造两个 parser
        ...
        match (opt1, opt2) {
            (Some(r1), Some(r2)) => {
                ...
                if r1_batch.len() == batch_len {
                    tx.send((r1_batch.split_off(0), r2_batch.split_off(0)))
                        .unwrap();
                }
...
fn main() -> Result<()> {
    ...
    let r1_output = PathBuf::from(format!(
        "{}_S1_L001_R1_{}{}",
        args.output_prefix, args.number_suffix, extension
    ));
    let r2_output = PathBuf::from(format!(
        "{}_S1_L001_R2_{}{}",
        args.output_prefix, args.number_suffix, extension
    ));
    let r3_output = PathBuf::from(format!(
        "{}_S1_L001_R3_{}{}",
        args.output_prefix, args.number_suffix, extension
    ));
    ...
    if args.verbose {
        println!(
            "Starting batch processing with batch size: {}",
            args.batch_size
        );
    }
    ...
    let (output_tx, output_rx): (Sender<Vec<ProcessedRecord>>, Receiver<Vec<ProcessedRecord>>) =
        bounded(50);
        ...
        thread::spawn(move || -> Result<()> {
            ...
            while let Ok(batch_results) = output_rx.recv() {
                ...
                if !r1_batch.is_empty() {
                    r1_tx_clone
                        .send(r1_batch)
                        .map_err(|_| anyhow::anyhow!("Failed to send R1 batch"))?;
                    r2_tx_clone
                        .send(r2_batch)
                        .map_err(|_| anyhow::anyhow!("Failed to send R2 batch"))?;
                    r3_tx_clone
                        .send(r3_batch)
                        .map_err(|_| anyhow::anyhow!("Failed to send R3 batch"))?;
                }
